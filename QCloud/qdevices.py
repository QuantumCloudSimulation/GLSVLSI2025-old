...
class IBM_QuantumDevice(QuantumDevice):
    ...
    def __init__(self, name, nodes_file_name, pos_file_name, env, maintenance_interval, maintenance_duration, maintenance_switch, clops, qvol, median_T1, median_T2, processor_type, cali_filepath=None, printlog=True):
        ...
        self.error_score = self.calculate_error_score()
    ...
    def extract_errors_from_csv(self):
        ...
        single_qubit_gate_errors = {
            "sx": calibration_data["√x (sx) error"].mean(),
            "x": calibration_data["Pauli-X error"].mean(),
        }
        two_qubit_gate_errors = {}
        
        # Check which column exists in the DataFrame
        error_column = None
        if "CZ error" in calibration_data.columns:
            error_column = "CZ error"
        elif "ECR error" in calibration_data.columns:
            error_column = "ECR error"

        clean_calibration_data = calibration_data.dropna(subset=[error_column])

        for cz_errors in clean_calibration_data[error_column]:            
    ...
    def estimate_fidelity(self, job):
        ...
        avg_single_qubit_error = self.single_qubit_gate_errors["sx"]
    ...
    def calculate_error_score(self): 
        return (0.4 * sum(self.readout_errors) / len(self.readout_errors)) + \
                        (0.3 * self.single_qubit_gate_errors["sx"]) + \
                        (0.3 * sum(self.two_qubit_gate_errors.values()) / len(self.two_qubit_gate_errors))
...
class IBM_rochester(IBM_QuantumDevice):
    ...
    Source: https://quantum-computing.ibm.com/
    """
    def __init__(self, env, name=None, cali_filepath=None, printlog=True):   
